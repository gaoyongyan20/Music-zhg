...
function setplaylistModel() {
   console.log("accept")
   content.mySongListmodel.clear()

   var allsongs=content.songlist.getAllMap()

   for (var i = 0; i <allsongs.length; i++) {
       if((playlistPath=songlist.getUrlByIndex(i))!==-1){
           var data = {
       ...
       content.mySongListmodel.append(data)
       content.getTitle(playlistPath, i)
   }
   content.mySongDatamodel.model = content.mySongListmodel
   content.mySongDatamodel.currentIndex = 0
}

function appendsong(cindex){

   var data={
       "filePath":content.mySongListmodel.get(cindex).playlistPath,
       "title":content.mySongListmodel.get(cindex).title,
       "author":content.mySongListmodel.get(cindex).author
   }
